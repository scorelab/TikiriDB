...
read_all(SOCKET sock, char *buf, uint64_t len)
{
  ...
  while(!done) {
    ...
    read += n;
    done = ((uint32_t)(len - read) > 0) ? 0 : 1;
  }
  LOG_DEBUG("read done : %lu\n", read);
  return read;
}
/*---------------------------------------------------------------------------*/
/*
...
connect_to_server(char *address, int port)
{
  ...
  for(p = servinfo; p != NULL; p = p->ai_next) {
    if((sockfd = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) == -1) {
      strerror_r(errno, error_str, MAX_ERROR_STR_SIZE);
      LOG_DEBUG("socket: Can not create a socket : %s\n", error_str);
      continue;
    }
    ...
    if(connect(sockfd, p->ai_addr, p->ai_addrlen) == -1) {
      close(sockfd);
      strerror_r(errno, error_str, MAX_ERROR_STR_SIZE);
      LOG_DEBUG("connect: Can not connect : %s\n", error_str);
      continue;
    }
  ...
  if(p == NULL)  {
    LOG_DEBUG("Could not connect to %s:%d\n", address, port);
    return -1;
  }
