...
screen_init(void)
{
  ...
  refresh(); // 1)
  wrefresh(mainwnd);
  screen = newwin(13, 100, 1, 1);
  screen2 = newwin(13, 27, 28, 20);
  box(screen, ACS_VLINE, ACS_HLINE);
  box(screen2, ACS_VLINE, ACS_HLINE);
}
...
update_display(void)
{
  ...
  curs_set(0);
  mvwprintw(screen2, 1, 1, "-------- HEADER --------");
  mvwprintw(screen2, 3, 6, "TIME: %d:%d:%d", now_hour, now_min, now_sec);
  mvwprintw(screen2, 5, 6, "DATE: %d-%d-%d", now_day, now_month, now_year);
  mvwprintw(screen2, 7, 6, "PRESS q TO END");
  mvwprintw(screen2, 10, 1, "-------- FOOTER --------");
  wrefresh(screen2);
